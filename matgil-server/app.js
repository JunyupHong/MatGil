...
(async () => {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TABLE Food (fID int, fName varchar(50), fCost int, fDesc varchar(500), recommendMenu varchar(3), seasonMenu varchar(3), bestMenu varchar(3), premiumMenu varchar(3), foodMaterial varchar(300), rID int, foreign key(rID) references RestArea(rID));
      CREATE TABLE Convenience (cID int, cName varchar(20), rID int, cDesc varchar(200), foreign key(rID) references RestArea(rID));
      CREATE TABLE Brand (bID int, bName varchar(20), rID int, bDesc varchar(200), foreign key(rID) references RestArea(rID));
    `);
    ...
    console.log(`[COMPLETE] INIT Highway Table - ${highwayData.length} tuples`);
    ...
    console.log(`[COMPLETE] INIT Restarea Table - ${req.length} tuples`);
    ...
    for (let i = 0; i < convenienceData.length; i++) {
      ...
      await client.query(`INSERT INTO Convenience (cID, cName, rID, cDesc) VALUES (${convenienceData[i].psCode}, '${convenienceData[i].psName}', ${convenienceData[i].stdRestCd}, '${convenienceData[i].psDesc !== null ? convenienceData[i].psDesc.replace(/[\'\;]/g, ' ') : "-"}');`);
    }
    console.log(`[COMPLETE] INIT Convenience Table - ${convenienceData.length - 1} tuples`);
    ...
    for (let i = 0; i < foodData.length; i++) {
      await client.query(`INSERT INTO Food (fID, fName, fCost, fDesc, recommendMenu, seasonMenu, bestMenu, premiumMenu, foodMaterial, rID) VALUES (${foodData[i].seq}, '${foodData[i].foodNm}', ${foodData[i].foodCost}, '${foodData[i].etc !== null ? foodData[i].etc.replace(/[\'\;\"]/g, ' ') : '-'}', '${foodData[i].recommendyn}', '${foodData[i].seasonMenu}', '${foodData[i].bestfoodyn}', '${foodData[i].premiumyn}', '${foodData[i].foodMaterial !== null ? foodData[i].foodMaterial.replace(/[\'\;]/g, ' ') : "-"}', ${foodData[i].stdRestCd});`);
    }
    console.log(`[COMPLETE] INIT Food Table - ${foodData.length} tuples`);
    ...
    for (let i = 0; i < brandData.length; i++) {
      await client.query(`INSERT INTO Brand (bID, bName, rID, bDesc) VALUES (${brandData[i].brdCode}, '${brandData[i].brdName}', ${brandData[i].stdRestCd}, '${brandData[i].brdDesc !== null ? brandData[i].brdDesc.replace(/[\'\;]/g, " ") : "-"}');`);
    }
    console.log(`[COMPLETE] INIT Brand Table - ${brandData.length} tuples`);
